...
/**
 * setup consumer prices overlay
 */
setup_consumer_prices_overlay: function setup_consumer_prices_overlay()
{
    if(!app.session) return;
    if(app.pixi_mode!="subject") return;
    ...
    const html_text = new PIXI.HTMLText({
        text: `
    ...
    container.addChild(html_text);

    container.position.set(20, 150);

    pixi_consumer_prices_overlay.container = container;
    pixi_consumer_prices_overlay.html_text = html_text;

    app.update_consumer_prices_overlay();

    pixi_app.stage.addChild(pixi_consumer_prices_overlay.container);
},

/**
 * update consumer prices overlay
 */
update_consumer_prices_overlay: function update_consumer_prices_overlay()
{
    if(!app.session) return;
